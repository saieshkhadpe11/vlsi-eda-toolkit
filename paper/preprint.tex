...
Both algorithms achieve overlap-free placements on the random benchmark. Key
qualitative differences are:

\begin{itemize}
  \item \textbf{SA} produces a compact layout with lower total HPWL (benefit of
        Metropolis-guided global search), but thermal distribution is not explicitly
        optimized.
...
The 3-phase convergence curve of PIAB-FP (Fig.~\ref{fig:convergence}) exhibits a
characteristic elbow at each phase transition: a rapid cost decrease in Phase~1
(overlap elimination), a plateau in Phase~2 (reorganization), and a smooth
decrease in Phase~3 (compaction).

\begin{figure}[h]
\centering
\fbox{\rule{0pt}{2cm}\rule{0.9\linewidth}{0pt}}
\caption{PIAB-FP convergence across three phases (cost vs. iteration).
Phase transitions are marked by dashed vertical lines.
(See \texttt{output/piab\_convergence.png} in the repository.)}
